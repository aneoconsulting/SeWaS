...
public:
  CentralFDOperator(const SWS::RealType dx,
                    const SWS::RealType dy,
                    const SWS::RealType dz)
    : dx_(dx)
    , dy_(dy)
    , dz_(dz)
  {
    /* ci coefficients are taken in "Simulating Seismic Wave Propagation in 3D
       Elastic Media Using Staggered-Grid Finite Differences" [Graves, 1996] */
...
private:
  /*
    The variables names (c1, c2, c3, c4) and (d1, d2, d3, d4) are the same used
    in the paper: "3D Elastic Finite-Difference Modeling of Seismic Motion Using
...
public:
  ...
  inline auto apply(const SWS::SpatialBlockField<SWS::RealType>& fijk,
                    const int i,
                    const int j,
                    const short DK) const noexcept
  {
    ...
    switch (d) {
      case SWS::X: {
        ...
        return c1 * fijk(i + d1, j) + c2 * fijk(i - d2, j) +
               c3 * fijk(i + d3, j) + c4 * fijk(i - d4, j);
#elif BOOST_SIMD_VECTORIZATION
        for (int kp = 0; kp < nkp; kp++) {
          auto rijkp = SWS::SpatialBlockField<SWS::RealType>::getPack(rij, kp);

          const auto fi1jkp =
            SWS::SpatialBlockField<SWS::RealType>::getPack(fijk(i + d1, j), kp);
          const auto fi2jkp =
            SWS::SpatialBlockField<SWS::RealType>::getPack(fijk(i - d2, j), kp);
          const auto fi3jkp =
            SWS::SpatialBlockField<SWS::RealType>::getPack(fijk(i + d3, j), kp);
          const auto fi4jkp =
            SWS::SpatialBlockField<SWS::RealType>::getPack(fijk(i - d4, j), kp);
        ...
        for (int k = kStart; k < kEnd; k++) {
          // 4 (*) + 3 (+) = 7 flops
          rij(k - kStart) = c1 * fijk(i + d1, j, k) + c2 * fijk(i - d2, j, k) +
                            c3 * fijk(i + d3, j, k) + c4 * fijk(i - d4, j, k);
        }
#endif
...
#ifdef EIGEN_VECTORIZATION
        return c1 * fijk(i, j + d1) + c2 * fijk(i, j - d2) +
               c3 * fijk(i, j + d3) + c4 * fijk(i, j - d4);
#elif BOOST_SIMD_VECTORIZATION
        for (int kp = 0; kp < nkp; kp++) {
          auto rijkp = SWS::SpatialBlockField<SWS::RealType>::getPack(rij, kp);

          const auto fij1kp =
            SWS::SpatialBlockField<SWS::RealType>::getPack(fijk(i, j + d1), kp);
          const auto fij2kp =
            SWS::SpatialBlockField<SWS::RealType>::getPack(fijk(i, j - d2), kp);
          const auto fij3kp =
            SWS::SpatialBlockField<SWS::RealType>::getPack(fijk(i, j + d3), kp);
          const auto fij4kp =
            SWS::SpatialBlockField<SWS::RealType>::getPack(fijk(i, j - d4), kp);
...
#else
        for (int k = kStart; k < kEnd; k++) {
          rij(k - kStart) = c1 * fijk(i, j + d1, k) + c2 * fijk(i, j - d2, k) +
                            c3 * fijk(i, j + d3, k) + c4 * fijk(i, j - d4, k);
        }
#endif
...
#ifdef EIGEN_VECTORIZATION
        return c1 * fijk.get(i, j, kStart + d1) +
               c2 * fijk.get(i, j, kStart - d2) +
               c3 * fijk.get(i, j, kStart + d3) +
               c4 * fijk.get(i, j, kStart - d4);
#else
        for (int k = kStart; k < kEnd; k++) {
          rij(k - kStart) = c1 * fijk(i, j, k + d1) + c2 * fijk(i, j, k - d2) +
                            c3 * fijk(i, j, k + d3) + c4 * fijk(i, j, k - d4);
        }
#endif
        break;
      }
  ...
  inline auto apply(const SWS::SpatialBlockField<SWS::RealType>& fijk) const
    noexcept
  {
    ...
    // TODO avoid the following temporary object (can we overwrite data for the
    // time step ts-1?)
