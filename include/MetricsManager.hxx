...
private:
  ...
  /* Origin of time */
  const std::chrono::high_resolution_clock::time_point oElapsedTime_ =
...
public:
  ...
  inline void stop(const std::string eid)
  {
    ...
    if (events_[tn].find(eid) != events_[tn].end()) {
      ...
      auto elapsed =
        get<Metric::ELAPSED, Measurement::STOP>(tn, eid) - get<Metric::ELAPSED, Measurement::START>(tn, eid);
  ...
  inline void show()
  {
    ...
    for (auto tn = 0; tn < nthreads; tn++) {
      for (auto& it : events_[tn]) {
        ...
        cmetrics_[tn].push_back(
          std::make_pair<std::string, ComputedMetrics>(std::move(sevent), std::move(cm)));
      }
    }
      ...
      for (auto tn = 0; tn < nthreads; tn++) {
        for (auto& it : cmetrics_[tn]) {
          ...
          for (auto metric : { ELAPSED, CPU }) {
            ...
            cm[metric][SD] += (cm_n[metric][SD] * cm_n[metric][SD] + cm_n[metric][AVG] * cm_n[metric][AVG]) *
                              cm_n[metric][NCALLS];
          }
        }
      } // tn
...
private:
  MetricsManager(const std::string applicationName, const std::initializer_list<std::string> sampledEvents);
  ~MetricsManager();
  ...
  inline void sort()
  {
    for (auto tn = 0; tn < cmetrics_.size(); tn++) {
      std::sort(cmetrics_[tn].begin(), cmetrics_[tn].end(), [](const auto& cm1, const auto& cm2) {
