...
public:
  SpatialBlockField()
    : SpatialBlockField(1, 1, 1)
  {}

  SpatialBlockField(const int nx, const int ny, const int nz)
    : nx_(nx)
    , ny_(ny)
    , nz_(nz)
  ...
  inline auto operator()(const int i, const int j) { return get(i, j, kStart()); }

  inline const auto operator()(const int i, const int j) const { return get(i, j, kStart()); }

  inline auto& operator()(const int i, const int j, const int k) { return data_(index(i, j, k)); }

  inline const auto& operator()(const int i, const int j, const int k) const { return data_(index(i, j, k)); }
  ...
  inline void plot1D(const int j0, const int k0, const std::string name) const
  {
#ifdef USE_MATPLOTLIB
  ...
  inline void plot2D(const int k0, const std::string name) const
  {
#ifdef USE_MATPLOTLIB
