...
public:
  ...
  inline void render() { renderingWindow_->Render(); }

private:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW // To force an object of this class to be allocated as aligned

  VisualizationManager(const int nt, const int nxx, const int nyy, const int nzz, const int nthreads);
  ...
  template<int FIELD>
  inline int activateActor(vtkActor* actor, vtkRenderer* renderer, const int& ts, const int& tid)
  {
    hsv in;
  ...
  template<int FIELD>
  inline int initActor(vtkActor* actor, vtkRenderer* renderer, const int& ii, const int& jj, const int& kk)
  {
  ...
  inline int initVideoWriter()
  {
    ...
    writer_->SetFileName(
      ("sewas." + std::to_string(P) + "-" + std::to_string(Q) + "-" + std::to_string(R) + ".avi").c_str());
  ...
  inline void setViewportRanges()
  {
      ...
      for (int i = 0; i < 2; i++) { // x,y
        ...
        switch (i) {
          case X:
            ...
            viewportRanges_[VELOCITY][d][X][MAX] =
              ((SWS::RealType)d + 1) * (1 - CLUSTER_PANEL_SIZE) / SWS::DIM;
            break;
          case Y:
    ...
    for (auto sc = 0; sc < SWS::NB_STRESS_FIELD_COMPONENTS; sc++) {
      ...
      for (int i = 0; i < 2; i++) { // x,y
        ...
        switch (i) {
          case X:
            viewportRanges_[STRESS][sc][X][MIN] =
              ((SWS::RealType)sc) * (1 - CLUSTER_PANEL_SIZE) / SWS::NB_STRESS_FIELD_COMPONENTS;
            viewportRanges_[STRESS][sc][X][MAX] =
              ((SWS::RealType)sc + 1) * (1 - CLUSTER_PANEL_SIZE) / SWS::NB_STRESS_FIELD_COMPONENTS;
            break;
          case Y:
    ...
    for (auto n = 0; n < SWS::NB_CLUSTER_NODES; n++) {
      ...
      for (int i = 0; i < 2; i++) { // x,y
        ...
        switch (i) {
          case X:
            viewportRanges_[CLUSTER][n][X][MIN] =
              (1 - CLUSTER_PANEL_SIZE) + (n % 2) * (CLUSTER_PANEL_SIZE) / 2;
            viewportRanges_[CLUSTER][n][X][MAX] =
              (1 - CLUSTER_PANEL_SIZE) + (n % 2 + 1) * (CLUSTER_PANEL_SIZE) / 2;
            break;
          case Y:
  ...
  inline SWS::ColorizationStrategies getColorizationStrategy(const int f) const
  {
    ...
    if (cs) {
      if (strcmp("CORE", cs) == 0) {
        ...
        LOG(SWS::LOG_WARN,
            "Unknown colorization strategy {}. Accepted values are CORE and TIME_STEP. Fallback to CORE",
            cs);
        return SWS::CORE;
      }
    } else {
  ...
  */
  ...
  int removeVelocityTask(const SWS::Directions& d, const int& ii, const int& jj, const int& kk);
  int desactivateVelocityTask(const SWS::Directions& d, const int& ii, const int& jj, const int& kk);
  ...
  int removeStressTask(const SWS::StressFieldComponents& sc, const int& ii, const int& jj, const int& kk);
  ...
  int removeCore(const SWS::ClusterNodes& n, const int& tid);
  int addCore(const SWS::ClusterNodes& n, const int& tid, const int& ts);
  int displayCore(const SWS::ClusterNodes& n, const int& tid, const int& ts);
