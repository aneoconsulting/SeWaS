...
ExternalSource * ExternalSource::getInstance(const std::string eSourceForceFile){
...
void ExternalSource::releaseInstance(){
  if (pInstance_){
    delete pInstance_;
    pInstance_ = nullptr;
  }
...
const SWS::SpatialField & ExternalSource::operator()(const SWS::Directions & d) const{
  return F_(d);
}

ExternalSource::ExternalSource(const std::string eSourceForceFile):eSourceForceFile_(eSourceForceFile)
{
...
int ExternalSource::intializeSourceForce(){
