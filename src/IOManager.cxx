...
IOManager::IOManager(const int nt, const int lnxx, const int lnyy, const int lnzz) : nt_(nt),
                                                                                     lnxx_(lnxx),
                                                                                     lnyy_(lnyy),
                                                                                     lnzz_(lnzz),
                                                                                     processedTasks_(nt)
...
IOManager &IOManager::getInstance(const int nt, const int lnxx, const int lnyy, const int lnzz)
{
...
#ifdef ENABLE_IO
int IOManager::dumpTile(adios2::Engine &writer, const SWS::SpatialBlockField<SWS::RealType> &tile3D, const std::string tileID)
{
    int status = 0;
    ...
    for (int ii = 0; ii < lnxx_; ii++)
    {
        for (int jj = 0; jj < lnyy_; jj++)
        {
            for (int kk = 0; kk < lnzz_; kk++)
            {
                ...
                {
                    auto tileID = "Velocity-" + getUID(d, ii, jj, kk);
                    io_.DefineVariable<SWS::RealType>(tileID, {(uint) world * lncells}, {(uint) rank * lncells}, {(uint) tileSize}, adios2::ConstantDims);
                }
