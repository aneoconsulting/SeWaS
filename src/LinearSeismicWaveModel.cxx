...
LinearSeismicWaveModel*
LinearSeismicWaveModel::getInstance()
{
...
int
LinearSeismicWaveModel::initializeFieldsWrapper(const int ii, const int jj, const int kk)
{
  ...
  LogManager::getInstance()->log<SWS::LOG_TRACE>(
    "Completed initializing data on tile ({}, {}, {})", ii, jj, kk);

  return 0;
}

int
LinearSeismicWaveModel::computeVelocityWrapper(const int d,
                                               const int ts,
                                               const int ii,
                                               const int jj,
                                               const int kk)
...
int
LinearSeismicWaveModel::computeVelocity(const SWS::Directions& d,
                                        const int& ts,
                                        const int& ii,
                                        const int& jj,
                                        const int& kk)
  ...
  switch (d) {
    case SWS::X: {
      ...
      for (int i = iStart; i < iEnd; i++) {
        for (int j = jStart; j < jEnd; j++) {
          vX(i, j) +=
            (fdo_.apply<SWS::X>(sigmaXX, i, j, SWS::DK) + fdo_.apply<SWS::Y>(sigmaXY, i, j, SWS::DK) +
             fdo_.apply<SWS::Z>(sigmaXZ, i, j, SWS::DK)) *
            dt * bx(i, j);
        }
      }
#else
      for (int i = iStart; i < iEnd; i++) {
        for (int j = jStart; j < jEnd; j++) {
          for (int k = kStart; k < kEnd; k++) {
            ...
            vx += (c3 * (sigmaXX(i, j, k) - sigmaXX(i - 1, j, k)) -
                   c2 * (sigmaXX(i + 1, j, k) - sigmaXX(i - 2, j, k))) /
                  ds; // Dx(sigmaXX)
            vx += (c3 * (sigmaXY(i, j, k) - sigmaXY(i, j - 1, k)) -
                   c2 * (sigmaXY(i, j + 1, k) - sigmaXY(i, j - 2, k))) /
                  ds; // Dy(sigmaXY)
            vx += (c3 * (sigmaXZ(i, j, k) - sigmaXZ(i, j, k - 1)) -
                   c2 * (sigmaXZ(i, j, k + 1) - sigmaXZ(i, j, k - 2))) /
                  ds; // Dz(sigmaXZ)
            vx *= dt * bx(i, j, k);
    ...
    case SWS::Y: {
      ...
      for (int i = iStart; i < iEnd; i++) {
        for (int j = jStart; j < jEnd; j++) {
          vY(i, j) +=
            (fdo_.apply<SWS::X>(sigmaXY, i + 1, j, SWS::DK) + fdo_.apply<SWS::Y>(sigmaYY, i, j + 1, SWS::DK) +
             fdo_.apply<SWS::Z>(sigmaYZ, i, j, SWS::DK)) *
            dt * by(i, j);
        }
      }
#else
      for (int i = iStart; i < iEnd; i++) {
        for (int j = jStart; j < jEnd; j++) {
          for (int k = kStart; k < kEnd; k++) {
            ...
            vy += (c3 * (sigmaXY(i + 1, j, k) - sigmaXY(i, j, k)) -
                   c2 * (sigmaXY(i + 2, j, k) - sigmaXY(i - 1, j, k))) /
                  ds; // Dx(sigmaXY)
            vy += (c3 * (sigmaYY(i, j + 1, k) - sigmaYY(i, j, k)) -
                   c2 * (sigmaYY(i, j + 2, k) - sigmaYY(i, j - 1, k))) /
                  ds; // Dy(sigmaYY)
            vy += (c3 * (sigmaYZ(i, j, k) - sigmaYZ(i, j, k - 1)) -
                   c2 * (sigmaYZ(i, j, k + 1) - sigmaYZ(i, j, k - 2))) /
                  ds; // Dz(sigmaYZ)
            vy *= dt * by(i, j, k);
    ...
    case SWS::Z: {
      ...
      for (int i = iStart; i < iEnd; i++) {
        for (int j = jStart; j < jEnd; j++) {
          vZ(i, j) +=
            (fdo_.apply<SWS::X>(sigmaXZ, i + 1, j, SWS::DK) + fdo_.apply<SWS::Y>(sigmaYZ, i, j, SWS::DK) +
             fdo_.apply<SWS::Z>(sigmaZZ, i, j, SWS::DK + 1)) *
            dt * bz(i, j);
        }
      }
#else
      for (int i = iStart; i < iEnd; i++) {
        for (int j = jStart; j < jEnd; j++) {
          for (int k = kStart; k < kEnd; k++) {
            ...
            vz += (c3 * (sigmaXZ(i + 1, j, k) - sigmaXZ(i, j, k)) -
                   c2 * (sigmaXZ(i + 2, j, k) - sigmaXZ(i - 1, j, k))) /
                  ds; // Dx(sigmaXZ)
            vz += (c3 * (sigmaYZ(i, j, k) - sigmaYZ(i, j - 1, k)) -
                   c2 * (sigmaYZ(i, j + 1, k) - sigmaYZ(i, j - 2, k))) /
                  ds; // Dy(sigmaYZ)
            vz += (c3 * (sigmaZZ(i, j, k + 1) - sigmaZZ(i, j, k)) -
                   c2 * (sigmaZZ(i, j, k + 2) - sigmaZZ(i, j, k - 1))) /
                  ds; // Dz(sigmaZZ)
            vz *= dt * bz(i, j, k);
...
int
LinearSeismicWaveModel::computeStressWrapper(const int sc,
                                             const int ts,
                                             const int ii,
                                             const int jj,
                                             const int kk)
...
int
LinearSeismicWaveModel::computeStress(const SWS::StressFieldComponents& sc,
                                      const int& ts,
                                      const int& ii,
                                      const int& jj,
                                      const int& kk)
  ...
  switch (sc) {
    case SWS::XX:
      ...
      for (int i = iStart; i < iEnd; i++) {
        for (int j = jStart; j < jEnd; j++) {
          sigmaXX(i, j) +=
            ((fdo_.apply<SWS::Y>(vY, i, j, SWS::DK) + fdo_.apply<SWS::Z>(vZ, i, j, SWS::DK)) * lambda(i, j) +
             (lambda(i, j) + 2.0 * mu(i, j)) * fdo_.apply<SWS::X>(vX, i + 1, j, SWS::DK)) *
            dt;
        }
      }
#else
      for (int i = iStart; i < iEnd; i++) {
        for (int j = jStart; j < jEnd; j++) {
          for (int k = kStart; k < kEnd; k++) {
            ...
            sxx += (c3 * (vY(i, j, k) - vY(i, j - 1, k)) - c2 * (vY(i, j + 1, k) - vY(i, j - 2, k))) /
                   ds; // Dy(vY)
            sxx += (c3 * (vZ(i, j, k) - vZ(i, j, k - 1)) - c2 * (vZ(i, j, k + 1) - vZ(i, j, k - 2))) /
                   ds; // Dz(vZ)
            sxx *= lambda(i, j, k);
            sxx += (lambda(i, j, k) + 2.0 * mu(i, j, k)) *
                   (c3 * (vX(i + 1, j, k) - vX(i, j, k)) - c2 * (vX(i + 2, j, k) - vX(i - 1, j, k))) / ds;
            sxx *= dt;
    ...
    case SWS::YY:
      ...
      for (int i = iStart; i < iEnd; i++) {
        for (int j = jStart; j < jEnd; j++) {
          sigmaYY(i, j) +=
            ((fdo_.apply<SWS::X>(vX, i + 1, j, SWS::DK) + fdo_.apply<SWS::Z>(vZ, i, j, SWS::DK)) *
               lambda(i, j) +
             (lambda(i, j) + 2.0 * mu(i, j)) * fdo_.apply<SWS::Y>(vY, i, j, SWS::DK)) *
            dt;
        }
      }
#else
      for (int i = iStart; i < iEnd; i++) {
        for (int j = jStart; j < jEnd; j++) {
          for (int k = kStart; k < kEnd; k++) {
            ...
            syy += (c3 * (vX(i + 1, j, k) - vX(i, j, k)) - c2 * (vX(i + 2, j, k) - vX(i - 1, j, k))) /
                   ds; // Dx(vX)
            syy += (c3 * (vZ(i, j, k) - vZ(i, j, k - 1)) - c2 * (vZ(i, j, k + 1) - vZ(i, j, k - 2))) /
                   ds; // Dz(vZ)
            syy *= lambda(i, j, k);
            syy += (lambda(i, j, k) + 2.0 * mu(i, j, k)) *
                   (c3 * (vY(i, j, k) - vY(i, j - 1, k)) - c2 * (vY(i, j + 1, k) - vY(i, j - 2, k))) / ds;
            syy *= dt;
    ...
    case SWS::ZZ:
      ...
      for (int i = iStart; i < iEnd; i++) {
        for (int j = jStart; j < jEnd; j++) {
          sigmaZZ(i, j) +=
            ((fdo_.apply<SWS::X>(vX, i + 1, j, SWS::DK) + fdo_.apply<SWS::Y>(vY, i, j, SWS::DK)) *
               lambda(i, j) +
             (lambda(i, j) + 2.0 * mu(i, j)) * fdo_.apply<SWS::Z>(vZ, i, j, SWS::DK)) *
            dt;
        }
      }
#else
      for (int i = iStart; i < iEnd; i++) {
        for (int j = jStart; j < jEnd; j++) {
          for (int k = kStart; k < kEnd; k++) {
            ...
            szz += (c3 * (vX(i + 1, j, k) - vX(i, j, k)) - c2 * (vX(i + 2, j, k) - vX(i - 1, j, k))) /
                   ds; // Dx(vX)
            szz += (c3 * (vY(i, j, k) - vY(i, j - 1, k)) - c2 * (vY(i, j + 1, k) - vY(i, j - 2, k))) /
                   ds; // Dy(vY)
            szz *= lambda(i, j, k);
            szz += (lambda(i, j, k) + 2.0 * mu(i, j, k)) *
                   (c3 * (vZ(i, j, k) - vZ(i, j, k - 1)) - c2 * (vZ(i, j, k + 1) - vZ(i, j, k - 2))) / ds;
            szz *= dt;
    ...
    case SWS::XY:
      ...
      for (int i = iStart; i < iEnd; i++) {
        for (int j = jStart; j < jEnd; j++) {
          sigmaXY(i, j) +=
            (fdo_.apply<SWS::Y>(vX, i, j + 1, SWS::DK) + fdo_.apply<SWS::X>(vY, i, j, SWS::DK)) * mu(i, j) *
            dt;
        }
      }
#else
      for (int i = iStart; i < iEnd; i++) {
        for (int j = jStart; j < jEnd; j++) {
          for (int k = kStart; k < kEnd; k++) {
            ...
            sxy += (c3 * (vX(i, j + 1, k) - vX(i, j, k)) - c2 * (vX(i, j + 2, k) - vX(i, j - 1, k))) /
                   ds; // Dy(vX)
            sxy += (c3 * (vY(i, j, k) - vY(i - 1, j, k)) - c2 * (vY(i + 1, j, k) - vY(i - 2, j, k))) /
                   ds; // Dx(vY)
            sxy *= mu(i, j, k) * dt;
    ...
    case SWS::XZ:
      ...
      for (int i = iStart; i < iEnd; i++) {
        for (int j = jStart; j < jEnd; j++) {
          sigmaXZ(i, j) +=
            (fdo_.apply<SWS::Z>(vX, i, j, SWS::DK + 1) + fdo_.apply<SWS::X>(vZ, i, j, SWS::DK)) * mu(i, j) *
            dt;
        }
      }
#else
      for (int i = iStart; i < iEnd; i++) {
        for (int j = jStart; j < jEnd; j++) {
          for (int k = kStart; k < kEnd; k++) {
            ...
            sxz += (c3 * (vX(i, j, k + 1) - vX(i, j, k)) - c2 * (vX(i, j, k + 2) - vX(i, j, k - 1))) /
                   ds; // Dz(vX)
            sxz += (c3 * (vZ(i, j, k) - vZ(i - 1, j, k)) - c2 * (vZ(i + 1, j, k) - vZ(i - 2, j, k))) /
                   ds; // Dx(vZ)
            sxz *= mu(i, j, k) * dt;
    ...
    case SWS::YZ:
      ...
      for (int i = iStart; i < iEnd; i++) {
        for (int j = jStart; j < jEnd; j++) {
          sigmaYZ(i, j) +=
            (fdo_.apply<SWS::Z>(vY, i, j, SWS::DK + 1) + fdo_.apply<SWS::Y>(vZ, i, j + 1, SWS::DK)) *
            mu(i, j) * dt;
        }
      }
#else
      for (int i = iStart; i < iEnd; i++) {
        for (int j = jStart; j < jEnd; j++) {
          for (int k = kStart; k < kEnd; k++) {
            ...
            syz += (c3 * (vY(i, j, k + 1) - vY(i, j, k)) - c2 * (vY(i, j, k + 2) - vY(i, j, k - 1))) /
                   ds; // Dz(vY)
            syz += (c3 * (vZ(i, j + 1, k) - vZ(i, j, k)) - c2 * (vZ(i, j + 2, k) - vZ(i, j - 1, k))) /
                   ds; // Dy(vZ)
            syz *= mu(i, j, k) * dt;
...
void
LinearSeismicWaveModel::addVelocitySource(const SWS::Directions& d,
                                          const int ts,
                                          const int ii,
                                          const int jj,
                                          const int kk)
{
  if (!v_(d)(ii, jj, kk).hasSource())
    return;
  ...
  switch (d) {
    case SWS::X: {
      ...
      LOG(SWS::LOG_TRACE,
          "[start] Updating velocity source for Vx at time-step {} on tile ({}, {}, {})",
          ts,
          ii,
          jj,
          kk);
      ...
      for (auto s = 0; s < nsrc; ++s) {
        ...
        auto j = js[s];
        auto k = ks[s];
        vX(i, j, k) -= a * ((l - 1) * dt - 1.2 / 7.) *
                       exp(-M_PI * M_PI * 7. * 7. * ((l - 1) * dt - 1.2 / 7.) * ((l - 1) * dt - 1.2 / 7.)) *
                       dt / rho(i, j, k);
      }

      LOG(SWS::LOG_TRACE,
          "[stop] Updating velocity source for Vx at time-step {} on tile ({}, {}, {})",
          ts,
          ii,
          jj,
          kk);
    ...
    case SWS::Y: {
      ...
      LOG(SWS::LOG_TRACE,
          "[start] Updating velocity source for Vy at time-step {} on tile ({}, {}, {})",
          ts,
          ii,
          jj,
          kk);
      ...
      for (auto s = 0; s < nsrc; ++s) {
        ...
        auto j = js[s];
        auto k = ks[s];
        vY(i, j, k) -= b * ((l - 1) * dt - 1.2 / 7.) *
                       exp(-M_PI * M_PI * 7. * 7. * ((l - 1) * dt - 1.2 / 7.) * ((l - 1) * dt - 1.2 / 7.)) *
                       dt / rho(i, j, k);
      }

      LOG(SWS::LOG_TRACE,
          "[stop] Updating velocity source for Vy at time-step {} on tile ({}, {}, {})",
          ts,
          ii,
          jj,
          kk);
...
void
LinearSeismicWaveModel::initialize(const int ii, const int jj, const int kk)
{
  auto* pMesh = Mesh3DPartitioning::getInstance();
...
void
LinearSeismicWaveModel::setVelocitySourceLocations()
{
  auto pMesh = Mesh3DPartitioning::getInstance();
  ...
  for (auto& source : pm_.sources()) {
    ...
    if (rank == Mesh3DPartitioning::getInstance()->rank_of(ii, jj, kk)) {
      ...
      LOG(SWS::LOG_INFO,
          "Found velocity source at cell ({}, {}, {}) within the tile ({}, {}, {})",
          is,
          js,
          ks,
          ii,
