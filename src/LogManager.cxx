...
{
  logFileName_ =
    "sewas_" + SWS::getDateTime() + "_" + SWS::getHostName() + "_" + std::to_string(SWS::getPID()) + ".log";
  ...
  logger_ =
    std::make_shared<spdlog::async_logger>("logger",
...
LogManager::setVerbosityLevel()
{
  ...
  if (vl) {
    if (strcmp("OFF", vl) == 0) {
      ...
      spdlog::warn("Unrecognized token {} when selecting verbosity level. Accepted values are: {}. Falling "
                   "back to INFO mode",
                   vl,
