...
VisualizationManager * VisualizationManager::getInstance(const int nt,
...
void VisualizationManager::releaseInstance()
{
  if (pInstance_){
    delete pInstance_;
    pInstance_ = nullptr;
...
int VisualizationManager::removeVelocityTask(const SWS::Directions & d,
...
int VisualizationManager::desactivateVelocityTask(const SWS::Directions & d,
                                                  const int & ii, const int & jj, const int & kk)
{
  auto & actor=VActors_(d)(ii,jj,kk);
  auto & renderer=VRenderers_[d];
...
int VisualizationManager::activateVelocityTask(const SWS::Directions & d,
                                               const int & ts,
                                               const int & ii, const int & jj, const int & kk,
                                               const int & tid)
{
...
int VisualizationManager::displayVelocity(const SWS::Directions & d,
					  const int & ts,
					  const int & ii, const int & jj, const int & kk,
					  const int & tid)
{
...
int VisualizationManager::removeStressTask(const SWS::StressFieldComponents & sc,
                                                const int & ii, const int & jj, const int & kk)
{
  /* In order to remove the task previously displayed at
     the same spatial location (during the previous time-step) */
...
int VisualizationManager::desactivateStressTask(const SWS::StressFieldComponents & sc,
                                                const int & ii, const int & jj, const int & kk)
{
#ifdef ENABLE_STRESS_RENDERING
  auto & actor=SActors_(sc)(ii,jj,kk);
...
int VisualizationManager::activateStressTask(const SWS::StressFieldComponents & sc,
                                             const int & ts,
                                             const int & ii, const int & jj, const int & kk,
                                             const int & tid)
{
...
int VisualizationManager::displayStress(const SWS::StressFieldComponents & sc,
					const int & ts,
					const int & ii, const int & jj, const int & kk,
					const int & tid)
{
...
int VisualizationManager::removeCore(const SWS::ClusterNodes & n,
                                     const int & tid)
{
  /* In order to remove the task previously displayed at
     the same spatial location (during the previous time-step) */
...
int VisualizationManager::addCore(const SWS::ClusterNodes & n,
                                  const int & tid,
                                  const int & ts)
{
  auto & actor=CActors_(n,tid);
...
int VisualizationManager::displayCore(const SWS::ClusterNodes & n,
                                      const int & tid,
                                      const int & ts)
{
...
VisualizationManager::VisualizationManager(const int nt,
					   const int nxx, const int nyy, const int nzz,
                                           const int nthreads):nt_(nt),
                                                               nxx_(nxx), nyy_(nyy), nzz_(nzz),
                                                               nthreads_(nthreads)
{
